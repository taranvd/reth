...
Dev testnet:
      ...
      --dev.constant-base-fee
          ...
          Disables base fee adjustment so every mined block inherits the parent's base fee. Useful when replaying already-signed transactions whose fee caps cannot be adjusted, where sustained full blocks would otherwise drive the base fee above those caps.
